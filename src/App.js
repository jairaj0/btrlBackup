...
function App() {
      ...
      if (data) {
        ...
        try {
          if (Object.keys(data)[0] === "trade") {
            ...
            Object.keys(data)[0] === "balance" &&
            usernoCheck(data.balance.users)
          ) {
            balanceObj[data.balance.currency_symbol.toUpperCase()] =
              data.balance.balance;
            check++
          } else if (Object.keys(data)[0] === "market_history") {
            setMarketHistory(data.market_history);
          } else if (
            Object.keys(data)[0] === "trade_history" &&
            usernoCheck(data.trade_history.users)
          ) {
            setTradeHistory(data.trade_history);
          } else if (
            Object.keys(data)[0] === "open_order" &&
            usernoCheck(data.open_order.users)
          ) {
  ...
  useEffect(() => {
    ...
    // console.log(balanceObj , "check balance");
    }
  }, [check]);
      ...
      <Router>
        <Routes>
          ...
          <Route
            ...
            exact
            element={
              states.isLogin ? <Balance /> : <SignIn _redirect="balance" />
            }
          />
          <Route
            path="/refferal"
            exact
            element={
              states.isLogin ? <Refferal /> : <SignIn _redirect="refferal" />
            }
          />
          <Route
            path="/history"
            exact
            element={
              states.isLogin ? <History /> : <SignIn _redirect="history" />
            }
          />
          ...
          <Route
            path="/security"
            exact
            element={states.isLogin ? <Security /> : <NoteFound />}
          />
          <Route
            path="/change-password"
            exact
            element={states.isLogin ? <ChangePassword /> : <NoteFound />}
          />

          <Route
            path="/signup"
            exact
            element={!states.isLogin ? <Register /> : <NoteFound />}
          />
          <Route
            path="/signin"
            exact
            element={!states.isLogin ? <SignIn /> : <NoteFound />}
          />
          <Route
            path="/forget-password"
            exact
            element={!states.isLogin ? <ForgetPassword /> : <NoteFound />}
          />
          <Route
            path="/reset-password/*"
            exact
            element={!states.isLogin ? <ResetPassword /> : <NoteFound />}
          />
          {/* *********** Secure ********** */}
